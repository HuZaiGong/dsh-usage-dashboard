...
'.uds-tooltip{position:fixed;z-index:100;pointer-events:none;background:var(--dsw-alias-bg-layer-3);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;box-shadow:var(--dsw-shadow-lv1);padding:8px 10px;font-size:11px;line-height:17px;color:var(--dsw-alias-label-primary);min-width:160px}',
'.uds-tooltipKey{margin-bottom:5px;font-weight:600;color:var(--dsw-alias-label-primary)}',
'.uds-tooltipRow{display:flex;justify-content:space-between;gap:16px;color:var(--dsw-alias-label-secondary)}',
'.uds-tooltipRow .uds-legendDot{flex:none;margin-top:5px}',
'.uds-tooltipVal{display:inline-flex;align-items:center;gap:5px;font-variant-numeric:tabular-nums}',
...
function TrendChart({ buckets, t }) {
  const [tip, setTip] = React.useState(null) // { b, x, y }
  ...
  const bars = buckets.map((b) => {
    ...
    return React.createElement('div', {
      key: b.key,
      className: 'uds-barCol',
      onMouseMove: (e) => setTip({ b: b, x: e.clientX, y: e.clientY }),
  ...
  if (tip) {
    const b = tip.b
    const rows = [
      { key: 'uncachedInput', label: t('legendUncached'), value: formatTokens(b.uncachedInput), cls: 'uds-segUncached' },
      { key: 'cacheRead', label: t('legendCacheRead'), value: formatTokens(b.cacheRead), cls: 'uds-segCacheRead' },
      { key: 'cacheWrite', label: t('legendCacheWrite'), value: formatTokens(b.cacheWrite), cls: 'uds-segCacheWrite' },
      { key: 'output', label: t('legendOutput'), value: formatTokens(b.output), cls: 'uds-segOutput' },
    ].filter((r) => b[r.key] !== undefined)
    const vw = typeof window !== 'undefined' ? window.innerWidth : 1024
    const vh = typeof window !== 'undefined' ? window.innerHeight : 768
    const left = Math.max(8, Math.min(tip.x + 14, vw - 190))
    const top = Math.max(8, Math.min(tip.y + 14, vh - 160))
    tooltip = React.createElement('div', { className: 'uds-tooltip', style: { left: left, top: top } },
      React.createElement('div', { className: 'uds-tooltipKey' }, b.key),
      rows.map((r) => React.createElement('div', { key: r.label, className: 'uds-tooltipRow' },
        React.createElement('span', null, r.label),
        React.createElement('span', { className: 'uds-tooltipVal' },
          React.createElement('span', { className: 'uds-legendDot ' + r.cls }),
          r.value,
        ),
      )),
      React.createElement('div', { className: 'uds-tooltipRow', style: { marginTop: 4, fontWeight: 600, color: 'var(--dsw-alias-label-primary)' } },
        React.createElement('span', null, t('calls')),
        React.createElement('span', null, String(b.calls)),
      ),
    )
  }
