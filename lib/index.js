...
import { discoverSessions, diffChanged, readSessionLog, readSessionLogJs, zstdAvailable } from './scan.js'
import { extractUsage, sumUsage, bucketByDay, bucketByHour, groupByModel, filterByRange, sessionMeta } from './aggregate.js'
import { DEFAULT_PRICES, estimateCost, lookupPrice, mergePrices, loadConfigPrices, fetchModelsDev } from './pricing.js'
...
export class UsageStatsGateway extends TypertRemoteService {
  ...
  constructor(ctx) {
    ...
    // 价格表：内置 + 用户配置文件（$DSH_HOME/usage-prices.json）+ 后台 models.dev 拉取
  ...
  async _doScan() {
    const t0 = Date.now()
    if (this._zstdOk === undefined) this._zstdOk = zstdAvailable()
...
export const name = '@huzaigong/dsh-usage-dashboard'
export const inject = []
