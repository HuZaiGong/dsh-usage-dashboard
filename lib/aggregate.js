 ...
 * - provider/model 取自 assistant/message 的 message.source；chunk-only 记录
 *   继承日志中最近一次已知的 provider/model（消除 ?/? 桶）
 * 返回 [{ turn, step, provider, model, uncachedInput, cacheRead, cacheWrite, output, reasoning, atMs }]
 */
export function extractUsage(lines) {
  const last = new Map() // key: turn:step
  ...
  for (const line of lines) {
    ...
    if (!ev) continue
    const d = ev.data || {}
    const key = d.turn !== undefined && d.step !== undefined ? d.turn + ':' + d.step : null
    let usage = null
