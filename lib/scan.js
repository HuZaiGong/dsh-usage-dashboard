...
/** fzstd 回退的最大输入字节数（与 CLI maxBuffer 512MB 对齐，防异常大文件内存爆炸）。 */
export const MAX_JS_DECODE_BYTES = 512 * 1024 * 1024

/** fzstd（纯 JS）解压回退：系统 zstd CLI 缺失时使用。完整流输出与 CLI 一致；
 *  写入中的不完整流可能少尾部数据（下次增量重读补齐）。超过 {@link MAX_JS_DECODE_BYTES} 抛错。 */
export function readSessionLogJs(file, onLine) {
  const stat = statSync(file)
  if (stat.size > MAX_JS_DECODE_BYTES) throw new Error('session log too large for js decoder: ' + stat.size + ' bytes')
  const out = decompress(new Uint8Array(readFileSync(file)))
